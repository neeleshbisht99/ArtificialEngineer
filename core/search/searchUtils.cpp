...
namespace SearchNet {
    ...
    Individual *SearchUtils::mutateIndividual(SearchNet::Individual &in) {
        // Clone the individual.
        auto offspring = in.clone();

        auto ran = SearchRandomFloat(0, 1);

        if (ran < in.args->p1) { // p1
            // Add the new node between a connection.
            auto nodeOld = (*(offspring->nodeMap))[(*(offspring->nodeSlice))[SearchRandomUint32_t(
            ...
            if (!nodeOld->genomeMap->empty()) {
                ...
                auto ranPos = SearchRandomUint32_t(uint32_t(nodeOld->genomeMap->size()));
                uint32_t count = 0;
            ...
            auto index = SearchRandomUint32_t(length);
            ...
            auto length = uint32_t(offspring->nodeSlice->size());
            auto index1 = SearchRandomUint32_t(length);
            auto index2 = index1 + SearchRandomUint32_t(length - index1);
            ...
            if (index1 != index2) {
                ...
                if (!((inputNode->nodeType == 0 && outputNode->nodeType == 0) || inputNode->nodeType == 2)) {
                    ...
                    if (outputNode->genomeMap->find(g) == outputNode->genomeMap->end()) {
                        Ome o(SearchRandomFloat(in.args->weightA, in.args->weightB), offspring->innovationNum);
                        offspring->innovationNum++;
            ...
            // Delete a connection.
            auto n = (*(offspring->nodeMap))[(*(offspring->nodeSlice))[SearchRandomUint32_t(
                    uint32_t(offspring->nodeSlice->size()))]];

            if (!(n->genomeMap->empty())) {
                Gen g;
                auto ranPos = SearchRandomUint32_t(uint32_t(n->genomeMap->size()));
                uint32_t count = 0;
    ...
    void SearchUtils::addHiddenNodes(SearchNet::Individual &in, uint32_t num) {
    ...
    void SearchUtils::addConnections(SearchNet::Individual &in, uint32_t num) {
        ...
        for (uint32_t i = 0; i < in.args->depth - 3; ++i) {
            split_arr.push_back(SearchRandomUint32_t(left));
        }
        ...
        for (uint32_t i = 0; i < num; ++i) {
            // Add a new connection between two nodes.
            auto index1 = SearchRandomUint32_t(in.args->depth);
            auto index2 = index1 + SearchRandomUint32_t(in.args->depth - index1);

            if (index1 != index2) {
                auto inputNum = arr[index1][SearchRandomUint32_t(arr[index1].size())];
                auto outputNum = arr[index2][SearchRandomUint32_t(arr[index2].size())];

                auto outputNode = (*(in.nodeMap))[outputNum];

                Gen g(inputNum, outputNum);
                if (outputNode->genomeMap->find(g) == outputNode->genomeMap->end()) {
                    Ome o(SearchRandomFloat(in.args->weightA, in.args->weightB), in.innovationNum);
                    in.innovationNum++;
                    outputNode->genomeMap->insert(std::make_pair(g, o));
                }
            }
